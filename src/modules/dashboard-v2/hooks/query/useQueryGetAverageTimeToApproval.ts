...
export const useQueryGetAverageTimeToApproval = ({
  filter,
  loanProgramIds
}: DashboardState) => {
  ...
  return useQuery<
    AxiosResponse<AverageTimeToApprovalResponse>,
  ...
  >({
    ...
    queryFn: async () => {
      ...
      return postRequest<RateRequest, AverageTimeToApprovalResponse>({
        path: API_PATH.dashboardV1.getAverageTimeToApproval(),
        data: {
          filter: {
