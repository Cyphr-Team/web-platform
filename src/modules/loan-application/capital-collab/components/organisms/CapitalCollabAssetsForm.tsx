...
export function CapitalCollabAssetsForm() {
  const { assets, dispatchFormAction } = useLoanApplicationFormContext()

  const form = useForm<CapitalCollabAssetsFormValue>({
    resolver: zodResolver(capitalCollabAssetsFormSchema),
    mode: "onBlur",
  ...
  const onSubmit = (data: CapitalCollabAssetsFormValue) => {
    dispatchFormAction({
