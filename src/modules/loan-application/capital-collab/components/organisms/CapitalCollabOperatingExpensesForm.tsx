...
import {
  capitalCollabOperatingExpensesFormSchema,
...
export function CapitalCollabOperatingExpensesForm() {
  const { fpOperatingExpenses, dispatchFormAction } =
    useLoanApplicationFormContext()

  const form = useForm<CapitalCollabOperatingExpensesFormValue>({
    resolver: zodResolver(capitalCollabOperatingExpensesFormSchema),
    mode: "onBlur",
...
interface OperatingExpensesProps {
  ...
  value: FieldArrayWithId<
    CapitalCollabOperatingExpensesFormValue["operatingExpenses"][number]
  >
...
function OperatingExpenses(props: OperatingExpensesProps) {
  ...
  const form = useFormContext<CapitalCollabOperatingExpensesFormValue>()

  // Apply the requirement, we can remove only when the items > 1
  const isRemovable =
    form.getValues(FpOperatingExpensesField.operatingExpenses).length > 0
        ...
        <div className="col-start-1 col-end-3 row-start-1 flex flex-col gap-1">
          <RHFTextInput
            ...
            name={getArrayFieldName<
              ...
              FieldPath<CapitalCollabOperatingExpensesFormValue>
